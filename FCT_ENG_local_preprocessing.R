# Preprocessing pipeline for Food Choice Task - ENG local version
# Author: Lucija Blazevski (l.blazevski@uva.nl), University of Amsterdam
# Date: 18-09-2023
# Use:   1. Place FCT_ENG_local_preprocessing.R script in the same directory as 
#        your data. Your data should be in the sub-folder called 'data' within
#        that directory.
#        2. Go to section 'Prepare work space' and replace the path to your 
#        directory that contains this script - FCT_ENG_local_preprocessing.R, 
#        and 'data' sub-folder with .csv files as mentioned in step 1. 
#        3. Go to section 'Options' and choose the type of output (See Notes for
#        further detail)
...
#              largely resembles the raw data and is saved in the
...
clean_df <- function(df) {
  ...
  choice_df <- choice_df %>% select("participant", "date", "condition", "order",
                                    ...
                                    "c_rating.rt", "chosen_item")
  rating_df$rating_keys.keys <- as.numeric(rating_df$rating_keys.keys)
  choice_df$c_rating.keys <- as.numeric(choice_df$c_rating.keys)
...
################################################################################
if (stacked_all) {
  all_participants_data <- bind_rows(data_combined, .id = "participant")
  all_participants_data <- as.data.frame(all_participants_data)
  
  all_participants_reshaped_data <- bind_rows(reshaped_data_list, .id = "participant")
